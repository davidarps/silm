...
class StructFormer_In_Parser(nn.Module):
  ...
  def generate_mask(self, x, distance, height):
    ...
    # The block_p, head_p each have size batch_size x (max_seq_length**3), which is enormous. Why necessary?
  ...
  def forward(self, x, pos):
    ...
    # att_mask corresponds to q_{i,j} in Eq. 16 in paper
...
class StructFormer_In_ParserModel(PreTrainedModel):
    ...
    def forward(self, input_ids, labels=None, **kwargs):
        ...
        return MaskedLTLMOutput(
          loss=loss,
          logits = sf_output[0].view(batch_size, length, -1),
          hidden_states=None,
          heads = sf_output[1]["head"]
        )
    def get_input_embeddings(self):
       return self.model.transformer_front.emb
