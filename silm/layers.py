...
class GatedMultiheadAttention(nn.Module):
    ...
    def forward(self, query, rels=None, key_padding_mask=None, attn_mask=None):
        ...
        # each has size batch_size x max_seq_length x embed_dim
        q, k, v, g = self.proj(query).chunk(4, dim=-1)

        # reshaping to the shape in the parameters. Essentially Eq. 7 with more convenient data structure
        ...
        # attn_output_weights: batch_size x max_seq_length x amx_seq_length x self.num_heads. Shape is kept through following computations. 
        ...
        # In this part, the Eq.s 8 and after are quite convoluted. Maybe rewrite equations
        ...
        attn_output_weights = (torch.log_softmax(
            attn_output_weights, dim=-1) + attn_mask[:, :, :, None]).exp() # result same size as above
        if key_padding_mask is not None:
        ...
        output = self.out_proj(self.drop(gated_output)) # The W_o and b_o parts of Eq.15. output is batch_size x max_seq_length x hidden_dim
